    ...
    window.BotemiaConfig = {
        ...
        "modules": {
            "preQualification": {
                ...
                "triggerPhraseLegacy": "I am ready to begin with the first question"
            },
        ...
        try {
            ...
            if (data.room_url && data.token) {
                ...
                console.log("✅ Joined Daily room (Server Connection Active)");
                               ...
                               // ===== 🔧 DEBUG: ENHANCED LISTENER WITH CHEAT CODES =====
                dailyCallObject.on("app-message", (ev) => {
                    
                    // 1. CHECK FOR "CHEAT CODES" IN THE CONSOLE
                    // You can type these in your browser console to force an action:
                    if (ev?.data?.type === "admin_command") {
                        if (ev.data.command === "FORCE_TRIGGER") {
                            console.log("🚀 [CHEAT] Manual Trigger Forced!");
                            forcePreQualification();
                        }
                        return;
                    }

                    // 2. SILENCE LOGIC (Keep this as is)
                    ...
                    // 3. MAIN TRANSCRIPTION LISTENER
                    if (ev?.data?.type === "agent_transcription") {
                        const tessText = ev.data.transcription;
                        
                        // === 🔍 DEBUG: VISUALIZER ===
                        // This creates a floating box on screen showing EXACTLY what the Bridge hears.
                        // If she talks and this stays empty, the audio pipe is broken.
                        let debuggerBox = document.getElementById("tess-debugger");
                        if (!debuggerBox) {
                            debuggerBox = document.createElement("div");
                            debuggerBox.id = "tess-debugger";
                            debuggerBox.style.cssText = "position: fixed; bottom: 10px; right: 10px; background: black; color: #0f0; padding: 10px; border: 2px solid #0f0; font-family: monospace; z-index: 999999; width: 300px; font-size: 12px;";
                            document.body.appendChild(debuggerBox);
                        }
                        debuggerBox.innerText = "🎧 HEARD: " + tessText;
                        // ===============================
                        ...
                        // We use a very simple check here to avoid fuzzy errors
                        const triggers = [
                            "are you ready for your first question", // The exact phrase
                        ...
                        if (hasTrigger) {
                            console.log("🎯 TRIGGER DETECTED! Phrase matched:", tessText);
                            forcePreQualification();
                        } else {
                            console.log("🚫 No trigger found in:", tessText);
                        }
                    }
                });
            } else {
